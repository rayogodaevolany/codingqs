...
char customString::operator[](int index){
    assert(index >= 0 && index < length(_head));
...
void customString::concatenate(const arrayString& b){
    ...
    // loop through b with the indexes set correctly
...
customString::arrayString customString::copiedString(const arrayString& original){
    int len; len = length(original);
    arrayString copy = new char[len + 1];
    copyCharacters(copy, original, len);
    // don't forget to add the terminating character
    copy[len] = 0;
    return copy;
}
